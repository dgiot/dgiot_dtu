...
public class MqttClientHelper {
  ...
  public void Publish(byte[] payload) {
    var appMsg = new MqttApplicationMessage(
        pubtopic, payload, MqttQualityOfServiceLevel.AtLeastOnce, false);
  ...
  private static async Task ConnectMqttServerAsync() {
    ...
    try {
      var options =
          new MqttClientTcpOptions { Server = server,     ClientId = clientid,
                                     UserName = username, Password = password,
                                     Port = port,         CleanSession = true };
  ...
  private static void MqttClient_Connected(object sender, EventArgs e) {
    ...
    mqttClient.SubscribeAsync(
        new TopicFilter(subtopic, MqttQualityOfServiceLevel.AtLeastOnce));
  ...
  /// <param name="sender"></param>
  /// <param name="e"></param>
  private static void MqttClient_ApplicationMessageReceived(
    ...
    LogHelper.Log("mqtt recv:topic: " + e.ApplicationMessage.Topic.ToString() +
                  " payload: " + data);

    Regex r_subtopic = new Regex(subtopic); // 定义一个Regex对象实例
    Match m_subtopic =
        r_subtopic.Match(e.ApplicationMessage.Topic); // 在字符串中匹配
    if (m_subtopic.Success) {
      SerialPortHelper.Write(e.ApplicationMessage.Payload, 0,
  ...
  public static void Write(byte[] data, int offset, int len) {
    if (bIsCheck) {
      var appMsg = new MqttApplicationMessage(
          pubtopic + clientid,
          Encoding.UTF8.GetBytes(LogHelper.Logdata(data, offset, len)),
          MqttQualityOfServiceLevel.AtLeastOnce, false);
      LogHelper.Log("mqtt client publish:" +
                    LogHelper.Logdata(data, offset, len));
      mqttClient.PublishAsync(appMsg);
    }
  }
  ...
  private static Dictionary<string, object> Get_payload(byte[] payload) {
    ...
    Dictionary<string, object> json =
        (Dictionary<string, object>)serializer.DeserializeObject(data);
    return json;
  }
  ...
  public static DateTime FromUnixTime(long timeStamp) {
    return TimeZone.CurrentTimeZone.ToLocalTime(
        new DateTime((timeStamp * 10000000) + BaseTime.Ticks));
  }
  ...
  public static long FromDateTime(DateTime dateTime) {
    return (TimeZone.CurrentTimeZone.ToUniversalTime(dateTime).Ticks -
            BaseTime.Ticks) /
           10000000;
  }
}
