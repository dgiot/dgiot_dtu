...
public partial class MainForm : Form {
  ...
  private static OPCDAViewHelper oPCDAViewHelper =
      OPCDAViewHelper.GetInstance();
  private static TcpClientHelper tcpClientHelper =
      TcpClientHelper.GetInstance();
  private static TcpServerHelper tcpServerHelper =
      TcpServerHelper.GetInstance();
  ...
  private readonly string[] bridges = new string[] {
    "SerialPort", "TcpServer", "BACnet",    "OPCUA",     "MqttClient",
    "MqttServer", "TcpClient", "TcpServer", "UdpClient", "UdpServer",
    "PLC",        "Control",   "Access",    "SqlServer", "Barcode_Printer"
  };
  ...
  public void Log(string text) {
    ...
    if (textBoxLog.Text.Length > 4096) {
      textBoxLog.Text =
          textBoxLog.Text.Substring(textBoxLog.Text.Length - 4096);
    }
  ...
  private void RestoreCommonConfig() {
    ...
    if (ConfigHelper.Check("ReconnectChecked")) {
      checkBoxReconnect.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("ReconnectChecked"));
    } else {
      ConfigHelper.SetConfig("ReconnectChecked",
                             DgiotHelper.BoolTostr(checkBoxReconnect.Checked));
    }
    ...
    if (ConfigHelper.Check("DisplayHex")) {
      checkBoxDisplayHex.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("DisplayHex"));
    } else {
      ConfigHelper.SetConfig("DisplayHex",
                             DgiotHelper.BoolTostr(checkBoxDisplayHex.Checked));
    }

    if (ConfigHelper.Check("Bridge_Checked")) {
      checkBoxBridge.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("Bridge_Checked"));
    } else {
      ConfigHelper.SetConfig("Bridge_Checked",
                             DgiotHelper.BoolTostr(checkBoxBridge.Checked));
    }
  }
  ...
  private void RestoreMqtt() {
    ...
    if (ConfigHelper.Check("MqttClient_Checked")) {
      radioButtonMqttClient.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("MqttClient_Checked"));
    } else {
      ConfigHelper.SetConfig(
          "MqttClient_Checked",
          DgiotHelper.BoolTostr(radioButtonMqttClient.Checked));
    }
  }
  ...
  private void RestoreTcp() {
    ...
    if (ConfigHelper.Check("TcpClient_Checked")) {
      radioButtonTcpClient.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("TcpClient_Checked"));
    } else {
      ConfigHelper.SetConfig(
          "TcpClient_Checked",
          DgiotHelper.BoolTostr(radioButtonTcpClient.Checked));
    }
  }
  ...
  private void RestoreUDP() {
    ...
    if (ConfigHelper.Check("UDPClient_Checked")) {
      radioButtonUDPClient.Checked =
          DgiotHelper.StrTobool(ConfigHelper.GetConfig("UDPClient_Checked"));
    } else {
      ConfigHelper.SetConfig(
          "UDPClient_Checked",
          DgiotHelper.BoolTostr(radioButtonUDPClient.Checked));
    }
  }
  ...
  private void Resh_Topic() {
    ...
    clientid =
        DgiotHelper.Md5("Device" + this.textBoxMqttUserName.Text + devaddr)
            .Substring(0, 10);
    textBoxMqttClientId.Text = clientid;
    textBoxMqttSubTopic.Text = "/" + productid + "/" + devaddr + "/device";
    textBoxMqttPubTopic.Text =
        "/" + productid + "/" + devaddr + "/properties/read/reply";
    textBoxAccessTopic.Text = "/" + productid + "/" + devaddr + "/scan/mdb";

    textBoxOPCUATopic.Text =
        "/" + productid + "/" + devaddr + "/scan/opcua/reply";
    textBoxPLCTopic.Text = "/" + productid + "/" + devaddr + "/scan/plc/reply";
    textBoxBACnetTopic.Text =
        "/" + productid + "/" + devaddr + "/scan/bacnet/reply";
    textBoxControlTopic.Text =
        "/" + productid + "/" + devaddr + "/scan/control/reply";
  }

  private void LinkLabel1LinkClicked(object sender,
  ...
  private void RadioButtonMqttClient_CheckedChanged(object sender,
                                                    EventArgs e) {
    ConfigHelper.SetConfig(
        "MqttClient_Checked",
        DgiotHelper.BoolTostr(radioButtonMqttClient.Checked));
  ...
  private void RadioButtonTcpClient_CheckedChanged(object sender, EventArgs e) {
    ConfigHelper.SetConfig("TcpClient_Checked",
                           DgiotHelper.BoolTostr(radioButtonTcpClient.Checked));
  ...
  private void RadioButtonUDPClient_CheckedChanged(object sender, EventArgs e) {
    ConfigHelper.SetConfig("UDPClient_Checked",
                           DgiotHelper.BoolTostr(radioButtonTcpClient.Checked));
    ConfigHelper.SetConfig("TcpClient_Checked", DgiotHelper.BoolTostr(false));
    ConfigHelper.SetConfig("MqttClient_Checked", DgiotHelper.BoolTostr(false));
  }
  ...
  private void ComboBoxCmdProdxy_SelectedIndexChanged(object sender,
                                                      EventArgs e) {
    ConfigHelper.SetConfig("CmdProdxy", comboBoxCmdProdxy.Text);
  }

  private void ComboBoxSerialPort_SelectedIndexChanged(object sender,
                                                       EventArgs e) {
    ConfigHelper.SetConfig("SerialPort", comboBoxSerialPort.Text);
  }

  private void ComboBoxBaudRate_SelectedIndexChanged(object sender,
                                                     EventArgs e) {
    ConfigHelper.SetConfig("BaudRate", comboBoxBaudRate.Text);
  }

  private void ComboBoxStopBits_SelectedIndexChanged(object sender,
                                                     EventArgs e) {
    ConfigHelper.SetConfig("StopBits", comboBoxStopBits.Text);
  }
  ...
  private void ComboBoxDataBits_SelectedIndexChanged(object sender,
                                                     EventArgs e) {
    ConfigHelper.SetConfig("DataBits", comboBoxDataBits.Text);
  }
  ...
  private void CheckBoxOPCDA_CheckedChanged(object sender, EventArgs e) {
    ConfigHelper.SetConfig("OPCDACheck",
                           DgiotHelper.BoolTostr(checkBoxOPCDA.Checked));
  ...
  private void ComboBoxLogLevel_SelectedIndexChanged(object sender,
                                                     EventArgs e) {
    ConfigHelper.SetConfig("LogLevel", comboBoxLogLevel.Text);
  ...
  private void ComboBoxDtuAddr_SelectedIndexChanged(object sender,
                                                    EventArgs e) {
    ConfigHelper.SetConfig("DtuAddr", comboBoxDtuAddr.Text);
  }
  ...
  private void SendBridge_Click(object sender, EventArgs e) {
    ...
    LogHelper.Log(bridges[comboBoxBridge.SelectedIndex] + " send  [" +
                  LogHelper.Logdata(payload, 0, payload.Length) + "]");
  ...
  private void NodeMouseDoubleClick(object sender,
    ...
    foreach (Control con in cons.Controls) {
      if (con.Tag != null && con.Tag != "") {
        string[] mytag = con.Tag.ToString().Split(
            new char[] { ':' }); // 获取控件的Tag属性值，并分割后存储字符串数组
        float a = Convert.ToSingle(mytag[0]) *
                  newx; // 根据窗体缩放比例确定控件的值，宽度
        con.Width = (int)a;                    // 宽度
